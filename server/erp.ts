...
const catalogRoles: BusinessRole[] = ["admin", "sales", "inventory"];
...
const staffDirectoryRoles: BusinessRole[] = [
  "admin",
  "payroll",
  "sales",
  "tailor",
];
const staffDocumentMimeTypes = new Set([
  "application/pdf",
  "image/png",
  "image/jpeg",
  "image/webp",
...
const staffDocumentExtensions = new Set([
  ".pdf",
  ".png",
  ".jpg",
  ".jpeg",
...
export const customPermissionValues = [
  "dashboard",
  "customers",
  "sales",
  "inventory",
...
export const customRoleCanAccess = (
  permissions: string[],
...
export const getDashboardRangeStart = (
  range: z.infer<typeof dashboardRange>,
  now = new Date()
) => {
...
export const getCustomDashboardRange = (
  startDate: string,
  endDate: string
) => ({
...
const dashboardInput = z
  ...
  .superRefine((value, ctx) => {
    if (value.range === "custom" && (!value.startDate || !value.endDate))
      ctx.addIssue({
        code: "custom",
        message: "Choose both a start and end date for a custom period.",
      });
    if (
      value.range === "custom" &&
      value.startDate &&
      ...
      ctx.addIssue({
        code: "custom",
        message:
          "The custom period end date must be on or after its start date.",
      });
  });
const salesSource = z.enum(["counter", "manual", "tailoring"]);
const salesHistoryInput = z
  .object({
    search: z.string().max(160).optional(),
    source: salesSource.optional(),
...
const invoiceListInput = z
  .object({
    search: z.string().max(160).optional(),
    status: z.enum(["paid", "partial", "unpaid"]).optional(),
    source: salesSource.optional(),
    paymentMethod: z
      .enum(["cash", "benefitpay", "bank_transfer", "credit_card"])
      .optional(),
    startDate: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/)
...
const invoiceDeleteInput = z.object({ invoiceId: z.number().int().positive() });
const manualSaleInput = z
  .object({
  ...
  .superRefine((value, ctx) => {
    if (value.discount > value.quantity * value.unitPrice)
      ctx.addIssue({
        code: "custom",
        path: ["discount"],
...
const monthInput = z.object({
  month: z
    .string()
    .regex(/^\d{4}-(0[1-9]|1[0-2])$/, "Choose a valid report month."),
});
...
async function access(
  ...
  if (assignment) {
    ...
    if (
      !assignment.isActive ||
      !role?.isActive ||
      !customRoleCanAccess(permissions, allowed)
    )
      throw new TRPCError({
        code: "FORBIDDEN",
        message:
          "Your owner-assigned role is not permitted to perform this action.",
      });
  ...
  throw new TRPCError({
    code: "FORBIDDEN",
    message: "Your assigned role is not permitted to perform this action.",
  });
}
...
export const tailoringOrderStatuses = [
  "draft",
  "confirmed",
  "cutting",
  "stitching",
...
const tailoringListInput = z.object({ search: z.string().max(160).optional() });
export const tailoringRelationError = (
  customerId: number,
  measurementCustomerId: number | null | undefined,
  tailorActive: boolean | null | undefined
) => {
  if (measurementCustomerId !== customerId) return "measurement" as const;
  if (tailorActive === false) return "tailor" as const;
  return null;
};
const tailoringOrderInput = z.object({
  customerId: z.number().int().positive(),
  measurementProfileId: z.number().int().positive().optional(),
  assignedTailorId: z.number().int().positive().optional(),
  garmentType: z.string().min(2).max(80),
  ...
  shop: router({
    ...
    save: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const values = {
          ...input,
          vatRate: input.vatRate.toFixed(3),
  ...
  dashboard: tenantProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      for (const sale of filteredSales) {
        ...
        revenueByDayMap.set(
          day,
          (revenueByDayMap.get(day) || 0) + Number(sale.total)
        );
      }
      ...
      const attendanceSummary = attendanceToday.reduce(
        ...
        { present: 0, absent: 0, leave: 0, half_day: 0 } as Record<
          string,
          number
        >
      );
      ...
      return {
        ...
        totalSales: total,
        saleCount,
        averageOrderValue: saleCount ? total / saleCount : 0,
        todayRevenue: Number(todayMetrics?.total || 0),
        weekRevenue: Number(weekMetrics?.total || 0),
        monthRevenue: Number(monthMetrics?.total || 0),
        lowStock: low,
        ...
        staff: {
          activeCount: Number(activeStaff?.count || 0),
  ...
  customers: router({
    ...
    balance: tenantProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const balance = rows.reduce(
          (sum, sale) =>
            sum + Math.max(0, Number(sale.total) - Number(sale.paidAmount)),
          0
        );
        return {
          customer,
          ...
          openInvoices: rows
            .filter(
              sale => Number(sale.total) - Number(sale.paidAmount) > 0.001
            )
            .map(sale => ({
              saleNumber: sale.saleNumber,
              total: Number(sale.total),
              paidAmount: Number(sale.paidAmount),
              outstanding: Number(sale.total) - Number(sale.paidAmount),
    ...
    sendBalance: tenantProcedure
      .input(
        z.object({
          customerId: z.number().int().positive(),
          channel: z.enum(["email", "whatsapp", "share"]),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!recipient)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: `Add a ${input.channel === "email" ? "customer email" : "customer phone number"} before sending the balance.`,
          });
        const balance = (
          await db
            .select()
        ...
        const message =
          input.message ||
          `Dear ${customer.name}, your current outstanding balance is ${balance.toFixed(3)} BHD.`;
        const result = await db
        ...
        await audit(
          ...
          "customer",
          customer.id,
          { channel: input.channel, recipient, balance }
        );
        return {
  ...
  inventory: router({
    ...
    create: tenantProcedure
      .input(
        z.object({
          code: z.string().min(1),
          name: z.string().min(2),
          inventoryType: z.enum(["material", "item"]).default("material"),
          category: z.enum([
            "fabric",
            "lining",
            "buttons",
          ...
          rollCount: z.number().int().min(0).default(0),
          metersPerRoll: z.number().positive().optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        const result = await db
          ...
          .values({
            ...
            quantity: three(openingQuantity),
            rollCount: input.rollCount,
            metersPerRoll: input.metersPerRoll?.toString() || null,
            minThreshold: three(input.minThreshold),
            costPerUnit: three(input.costPerUnit),
            salePrice: three(input.salePrice),
          })
        ...
        if (openingQuantity)
          await db
            ...
            .values({
              ...
              quantityAfter: three(openingQuantity),
              createdBy: ctx.user.id,
          ...
          if (seedQuantity !== null && seedQuantity > 0)
            await tx
              ...
              .values({
                ...
                quantityBefore: three(quantityBefore),
                quantityAfter: three(seedQuantity),
                createdBy: ctx.user.id,
                notes:
                  "Opening balance derived from roll count × meters per roll",
              });
        });
        await audit(
          ctx.user.id, ctx.organizationId,
          ...
          input.id,
          {
            ...
            quantityDerivedFromRolls: seedQuantity !== null,
          }
        );
        return { success: true };
      }),
    adjust: tenantProcedure
      .input(
        z.object({
          ...
          rollCountChange: z.number().int().default(0),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const rollQuantityChange =
          item.inventoryType === "material" &&
          input.quantityChange === 0 &&
          input.rollCountChange !== 0 &&
          Number(item.metersPerRoll) > 0
            ? input.rollCountChange * Number(item.metersPerRoll)
            : 0;
        const effectiveQuantityChange =
          input.quantityChange !== 0
            ? input.quantityChange
            : rollQuantityChange;
        const after = before + effectiveQuantityChange;
        const rollCountAfter =
          Number(item.rollCount || 0) + input.rollCountChange;
        if (effectiveQuantityChange === 0 && input.rollCountChange === 0)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Enter a meter or roll adjustment.",
          });
        if (after < 0 || rollCountAfter < 0)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Stock cannot drop below zero.",
          });
        await db
          .update(inventoryItems)
        ...
        await db
          ...
          .values({
            ...
            quantityBefore: three(before),
            quantityAfter: three(after),
            createdBy: ctx.user.id,
            notes: input.notes || null,
          });
        await audit(
    ...
    remove: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return { success: true, id: input.id, name: item.name };
      }),
  }),
  services: router({
    list: tenantProcedure.query(async ({ ctx }) => {
    ...
    create: tenantProcedure
      .input(
        z.object({
          ...
          defaultFabricMeters: z.number().optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        await access(ctx.user.id, ctx.organizationId, inventoryRoles);
        ...
        const saleId = await db.transaction(async tx => {
          const saleResult = await tx
            ...
            .values({
              ...
              subtotal: three(subtotal),
              discount: three(input.discount),
              total: three(total),
          ...
          for (const item of input.items) {
            await tx
              ...
              .values({
                ...
                quantity: three(item.quantity),
                unitPrice: three(item.unitPrice),
                lineTotal: three(item.quantity * item.unitPrice),
              });
            if (item.inventoryItemId) {
              ...
              if (!stock)
                throw new TRPCError({
                  code: "BAD_REQUEST",
                  message: "Linked inventory item was not found.",
                });
              ...
              if (after < 0)
                throw new TRPCError({
                  code: "BAD_REQUEST",
                  message: `${stock.name} does not have enough stock.`,
                });
              await tx
                .update(inventoryItems)
        ...
        return saleRows
          .filter(
            sale =>
              ...
              (!start || sale.createdAt >= start) &&
              (!end || sale.createdAt <= end) &&
              (!search ||
                [
                  sale.saleNumber,
    ...
    monthlyReport: tenantProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        for (const line of allItems)
          if (includedIds.has(line.saleId)) {
            const current = topLines.get(line.nameSnapshot) || {
              quantity: 0,
              total: 0,
            };
        ...
        return {
          ...
          totals: {
            ...
            revenue: total,
            averageOrder: rows.length ? total / rows.length : 0,
            paidCount: rows.filter(sale => sale.paymentStatus === "paid")
              .length,
            partialCount: rows.filter(sale => sale.paymentStatus === "partial")
              .length,
            unpaidCount: rows.filter(sale => sale.paymentStatus === "unpaid")
              .length,
          },
          bySource: Array.from(bySource, ([source, values]) => ({
            source,
            ...values,
          })),
          byPayment: Array.from(byPayment, ([paymentMethod, values]) => ({
            paymentMethod,
            ...values,
          })),
          topLines: Array.from(topLines, ([name, values]) => ({
            name,
            ...values,
          }))
  ...
  invoices: router({
    list: tenantProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        return invoiceRows
          .map(invoice => ({
            ...invoice,
            sale: saleById.get(invoice.saleId) || null,
          }))
          .filter(
            row =>
              ...
              (!start || row.issuedAt >= start) &&
              (!end || row.issuedAt <= end) &&
              (!search ||
                [
                  row.invoiceNumber,
                  row.sale?.saleNumber || "",
                  row.sale?.customerNameSnapshot || "",
                  row.sale?.customerPhoneSnapshot || "",
                ].some(value => value.toLowerCase().includes(search)))
          );
      }),
    detail: tenantProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (!sale)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Sale for this invoice was not found.",
          });
        const items = await db
          .select()
          .from(saleItems)
    ...
    addPayment: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const result = await db.transaction(async tx => {
          ...
          const amount = Number(input.amount.toFixed(3));
          const remainingBefore = Math.max(0, total - previousPaid);
          if (remainingBefore <= 0.001)
            throw new TRPCError({
              code: "BAD_REQUEST",
          ...
          if (amount > remainingBefore + 0.001)
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: `Payment cannot exceed the remaining balance of ${remainingBefore.toFixed(3)} BHD.`,
            });
          const paidTotal = Math.min(total, previousPaid + amount);
          const remainingAmount = Math.max(0, total - paidTotal);
          const nextStatus =
          ...
          const paymentResult = await tx
            ...
            .values({
              ...
              previousPaidAmount: previousPaid.toFixed(3),
              paidTotal: paidTotal.toFixed(3),
          ...
          await tx
            ...
            .values({
              ...
              amount: amount.toFixed(3),
              reference: input.reference || `${invoice.invoiceNumber} payment`,
              createdBy: ctx.user.id,
            });
          await tx
            .update(sales)
            .set({
              paidAmount: paidTotal.toFixed(3),
          ...
          return {
            ...
            sale,
            payment: {
              id: Number(paymentResult[0]?.id || 0),
              invoiceId: invoice.id,
              ...
              paymentMethod: input.paymentMethod,
              reference: input.reference || null,
              createdAt: paymentResult[0]?.createdAt || new Date(),
            },
            total,
            paidTotal,
            remainingAmount,
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const result = await db.transaction(async tx => {
          ...
          for (const line of stockLines) {
            ...
            const quantityAfter = quantityBefore + Number(line.quantity);
            await tx
              .update(inventoryItems)
            ...
            await tx
              ...
              .values({
                ...
                quantityBefore: three(quantityBefore),
                quantityAfter: three(quantityAfter),
                referenceType: "invoice_delete",
                referenceId: invoice.id,
                createdBy: ctx.user.id,
                notes: `Restored stock after deleting ${invoice.invoiceNumber}`,
              });
          }
          await tx
          ...
          return {
            invoiceNumber: invoice.invoiceNumber,
            saleNumber: sale.saleNumber,
            restoredStockLines: stockLines.length,
          };
        });
        await audit(
          ctx.user.id, ctx.organizationId,
    ...
    prepareDelivery: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return { id: deliveryId, status: "prepared" as const };
      }),
  }),
  staff: router({
    documents: router({
        ...
        .input(
          z
            .object({
              ...
              dataBase64: z.string().min(1).max(5000000),
            })
            .superRefine((value, ctx) => {
              const extension = value.fileName
                .toLowerCase()
                .slice(value.fileName.lastIndexOf("."));
              if (
                !staffDocumentMimeTypes.has(value.contentType) ||
                !staffDocumentExtensions.has(extension)
              )
                ctx.addIssue({
                  code: "custom",
                  path: ["fileName"],
                  message:
                    "Upload a PDF, PNG, JPG, WEBP, DOC, or DOCX staff document.",
                });
            })
        )
        .mutation(async ({ ctx, input }) => {
          ...
          const stored = await storagePut(
            `staff/${input.staffProfileId}/${Date.now()}-${safeName}`,
          ...
          await audit(
            ...
            "staffDocument",
            documentId,
            { staffProfileId: input.staffProfileId, label: input.label }
          );
    ...
    linkAccess: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!user)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "ERP access account not found.",
          });
        const existing = (
          await db
            .select({ id: staffProfiles.id })
        ...
        if (existing && existing.id !== input.staffProfileId)
          throw new TRPCError({
            code: "CONFLICT",
            message:
              "This ERP account is already linked to another payroll profile.",
          });
        await db
    ...
    recordAttendance: tenantProcedure
      .input(
        z.object({
          ...
          absenceReason: z
            .enum(["sick", "unexcused", "personal", "other"])
            .optional(),
          absenceDetails: z.string().max(1000).optional(),
          absenceDocumentId: z.number().int().positive().optional(),
          notes: z.string(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        const absenceDeduction = absenceDeductionFor(
        ...
        await db
          ...
          .values({
            ...
            status: input.status,
            absenceReason:
              input.status === "present" ? null : input.absenceReason || null,
            absenceDetails:
              input.status === "present"
                ? null
                : input.absenceDetails?.trim() || null,
            absenceDocumentId:
              input.status === "present"
                ? null
                : input.absenceDocumentId || null,
            absenceDeduction: three(absenceDeduction),
            notes: input.notes || null,
        ...
        await audit(
          ...
          "attendance",
          input.staffProfileId,
          { status: input.status, absenceDeduction }
        );
    ...
    removePayout: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await audit(
          ...
          "salaryPayout",
          input.id,
          { payslipNumber: payout.payslipNumber }
        );
    ...
    calculate: tenantProcedure
      .input(
        z.object({
          ...
          payPeriod: z
            .string()
            .regex(/^\d{4}-(0[1-9]|1[0-2])$/, "Choose a valid pay period."),
          allowances: z.number().min(0).max(1000000).default(0),
          overtime: z.number().min(0).max(1000000).default(0),
      ...
      .query(async ({ ctx, input }) => {
        ...
        const performanceBonus = rows.reduce(
          (sum, row) => sum + Number(row.commissionEarned),
          0
        );
        const baseSalary = Number(staff.baseSalary);
        const absenceDeduction = attendanceRows.reduce(
          (sum, row) =>
            sum +
            (Number(row.absenceDeduction) ||
              absenceDeductionFor(baseSalary, row.status, input.payPeriod)),
          0
        );
        const totalDeductions = input.deductions + absenceDeduction;
        const grossSalary =
          baseSalary + input.allowances + input.overtime + performanceBonus;
        ...
        return {
          ...
          performanceBonus,
          manualDeductions: input.deductions,
          absenceDeduction,
          deductions: totalDeductions,
          absenceCount: attendanceRows.filter(row => row.status === "absent")
            .length,
          halfDayCount: attendanceRows.filter(row => row.status === "half_day")
            .length,
          grossSalary,
          netSalary,
        };
    ...
    createPayout: tenantProcedure
      .input(
        z.object({
          ...
          allowances: z.number().min(0).max(1000000).default(0),
          overtime: z.number().min(0).max(1000000).default(0),
          performanceBonus: z.number().min(0).max(1000000).optional(),
          deductions: z.number().min(0).max(1000000).default(0),
          deductionDetails: z.string().max(2000).optional(),
          notes: z.string().max(2000),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        const performanceBonus =
          ...
          rows.reduce((sum, row) => sum + Number(row.commissionEarned), 0);
        const baseSalary = Number(staff.baseSalary);
        const absenceDeduction = attendanceRows.reduce(
          (sum, row) =>
            sum +
        ...
        if (finalAmount <= 0)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "The calculated net salary must be greater than zero.",
          });
        const payslipNumber = `PS-${input.payPeriod.replace("-", "")}-${staff.id}-${Date.now()}`;
        const deductionDetails =
          [
            input.deductionDetails?.trim(),
            absenceDeduction > 0
              ? `Automatic absence deduction: ${three(absenceDeduction)}`
              : "",
          ]
            .filter(Boolean)
            .join(" · ") || null;
        const result = await db
          ...
          .values({
            ...
            allowances: three(input.allowances),
            overtime: three(input.overtime),
            performanceBonus: three(performanceBonus),
            deductions: three(totalDeductions),
            deductionDetails,
            netSalary: three(finalAmount),
    ...
    list: tenantProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const measurements = new Map(
          measurementRows.map(profile => [profile.id, profile])
        );
        ...
        const mappedOrders = orders.map(order => ({
          ...order,
          sale: order.saleId ? salesById.get(order.saleId) || null : null,
          invoice: order.saleId
            ? invoicesBySaleId.get(order.saleId) || null
            : null,
          customerName:
            clients.get(order.customerId)?.name || "Archived customer",
          customerPhone: clients.get(order.customerId)?.phone || null,
          tailorName: order.assignedTailorId
            ? tailors.get(order.assignedTailorId)?.name || "Former tailor"
            : null,
          measurementVersion: order.measurementProfileId
            ? measurements.get(order.measurementProfileId)?.version || null
            : null,
        }));
        const search = input?.search?.trim().toLowerCase();
    ...
    create: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!customer)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Choose a valid customer for the tailoring order.",
          });
        const profile = input.measurementProfileId
          ...
          : null;
        const tailor = input.assignedTailorId
          ? (
              await db
                .select()
                .from(staffProfiles)
          ...
          : null;
        const relationError = tailoringRelationError(
          input.customerId,
          profile?.customerId,
          input.assignedTailorId ? Boolean(tailor?.isActive) : null
        );
        if (relationError === "measurement")
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Save or choose a measurement version belonging to this customer before confirming the tailoring order.",
          });
        if (relationError === "tailor")
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Choose an active tailor.",
          });
  ...
  team: router({
    listRoles: tenantProcedure.query(async ({ ctx }) => {
      ...
      const baseRoles = await db
        .select({
          ...
          businessRole: userBusinessRoles.role,
      ...
      return baseRoles.map(base => {
        ...
        const customRole = assignment
          ? byId.get(assignment.customRoleId)
          : undefined;
        return {
          ...base,
          customRoleId: customRole?.id || null,
          customRoleName: customRole?.name || null,
          customRoleActive: Boolean(
            assignment?.isActive && customRole?.isActive
          ),
        };
      });
    }),
    removeUser: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (target.role === "admin" || targetBusinessRole?.role === "admin")
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Owner accounts cannot be removed.",
          });
        await db.transaction(async tx => {
          await tx
    ...
    createCustomRole: tenantProcedure
      .input(
        z.object({
          ...
          permissions: z.array(z.enum(customPermissionValues)).min(1),
        })
      )
      .mutation(async ({ ctx, input }) => {
        await access(ctx.user.id, ctx.organizationId, adminRoles);
    ...
    assignCustomRole: tenantProcedure
      .input(
        z.object({
          userId: z.number().int().positive(),
          customRoleId: z.number().int().positive(),
          isActive: z.boolean(),
        })
      )
      .mutation(async ({ ctx, input }) => {
    ...
    assignRole: tenantProcedure
      .input(
        z.object({
          userId: z.number().int(),
          role: z.enum(["admin", "sales", "tailor", "inventory", "payroll"]),
          isActive: z.boolean(),
        })
      )
      .mutation(async ({ ctx, input }) => {
  ...
  access: router({
    ...
    listPending: tenantProcedure.query(async ({ ctx }) => {
      ...
      return requests.map(request => ({
        ...request,
        name: byId.get(request.userId)?.name || `User #${request.userId}`,
    ...
    approvePending: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!request || request.status !== "pending")
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "This access request is no longer waiting for review.",
          });
        if (!role?.isActive)
          throw new TRPCError({
            code: "BAD_REQUEST",
    ...
    approveWithPermissions: tenantProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await audit(
          ...
          "pendingAccessRequest",
          request.id,
          { userId: request.userId, roleId: transaction }
        );
